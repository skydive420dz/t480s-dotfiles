...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    vec4 trailColorAccent = vec4(clamp(iCurrentCursorColor.rgb * 1.35, 0.0, 1.0), 1.0);
    ...
    float mod = .007;
    //trailblaze
    vec4 trail = mix(trailColorAccent, fragColor, 1. - smoothstep(0., sdfTrail + mod, 0.007));
    trail = mix(trailColor, trail, 1. - smoothstep(0., sdfTrail + mod, 0.006));
    trail = mix(trail, trailColor, step(sdfTrail + mod, 0.));
    //cursorblaze
    trail = mix(trailColorAccent, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.004));
    trail = mix(trailColor, trail, 1. - smoothstep(0., sdfCurrentCursor + .002, 0.004));
